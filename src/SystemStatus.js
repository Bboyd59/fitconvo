  ...
  return (
    <div className="mt-4 border-t border-green-400 pt-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>CPU: <span className="text-red-400">{getRandomStatus()}</span></div>
        <div>MEMORY: <span className="text-blue-400">{getRandomStatus()}</span></div>
        <div>NETWORK: <span className="text-purple-400">{getRandomStatus()}</span></div>
        <div>SECURITY: <span className="text-orange-400">{getRandomStatus()}</span></div>
      </div>
      <div className="mt-2">
